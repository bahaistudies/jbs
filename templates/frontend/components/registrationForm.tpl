...
<div class="row justify-content-md-center">
    <div class="col-lg-6">
        <fieldset class="identity">
            ...
            <input class="form-control" type="text" name="givenName" id="givenName" value="{$givenName|escape}"
                maxlength="255" required aria-required="true" autofocus autocomplete="given-name">
    ...
    <div class="col-lg-6">
        <fieldset class="login">
            ...
            <input class="form-control" type="email" name="email" id="email" value="{$email|escape}" maxlength="90"
                required aria-required="true" autocomplete="email">
            <label for="username" class="required">
                {translate key="user.username"}
            </label>
            <input class="form-control" type="text" name="username" id="username" value="{$username|escape}"
                maxlength="32" required aria-required="true" autocomplete="username">
            <label for="password" class="required">
                {translate key="user.password"}
            </label>
            <input class="form-control" type="password" name="password" id="password" password="true" maxlength="32"
                required aria-required="true" autocomplete="new-password">
            <label for="password2" class="required">
                {translate key="user.repeatPassword"}
            </label>
            <input class="form-control" type="password" name="password2" id="password2" password="true" maxlength="32"
                ...
                {if $currentContext->getSetting('privacyStatement')}
                    ...
                    <div class="fields">
                        <div class="optin optin-privacy">
                            <label>
                                <input type="checkbox" name="privacyConsent" value="1" {if $privacyConsent} checked="checked"
                                    {/if}>
            ...
            {if $userCanRegisterReviewer}
                <fieldset class="reviewer">
                        ...
                        {foreach from=$reviewerUserGroups[$contextId] item=userGroup}
                            {if $userGroup->getPermitSelfRegistration()}
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input if-reviewer-checkbox"
                                        name="reviewerGroup[{$userGroup->getId()}]" id="reviewerGroup-{$userGroup->getId()}" value="1"
                                        {if in_array($userGroup->getId(), $userGroupIds)} checked="checked" {/if}>
                    ...
                    <div id="reviewerInterests" class="reviewer_interests hidden">
                        ...
                        <ul id="tagitInput" class="interests tag-it" data-field-name="interests[]"
